	...
	class Storefront {
		...
		public function widgets_init() {
			...
			$sidebar_args['header'] = array(
				...
				'id'          => 'header-1',
				'description' => __( '添加到该区域的窗口小部件将出现在页眉下方和主内容的上方。Widgets added to this region will appear beneath the header and above the main content.', 'storefront' ),
			);
			...
			for ( $row = 1; $row <= $rows; $row++ ) {
				for ( $region = 1; $region <= $regions; $region++ ) {
					...
					if ( 1 == $rows ) {
						...
						$footer_region_description = sprintf( __( '这里添加的小部件将出现在列中 %1$d 页脚。', 'storefront' ), $region );
					} else {
						$footer_region_name = sprintf( __( 'Footer Row %1$d - Column %2$d', 'storefront' ), $row, $region );
						$footer_region_description = sprintf( __( '这里添加的小部件将出现在列中。%1$d 页脚行 %2$d.', 'storefront' ), $region, $row );
					}
