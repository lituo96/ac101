...
if ( ! class_exists( 'Storefront_NUX_Guided_Tour' ) ) :
	...
	class Storefront_NUX_Guided_Tour {
				...
				<div class="sf-guided-tour-step">
					...
					<a class="sf-nux-button" href="#">
						<# if ( data.button_text ) { #>
							{{ data.button_text }}
						<# } else { #>
							<?php esc_attr_e( '接下来Next', 'storefront' ); ?>
						<# } #>
					</a>
					<# if ( ! data.last_step ) { #>
						<a class="sf-guided-tour-skip" href="#">
						<# if ( data.first_step ) { #>
							<?php esc_attr_e( '不，谢谢，跳过旅游No thanks, skip the tour', 'storefront' ); ?>
						<# } else { #>
							<?php esc_attr_e( '跳过这一步Skip this step', 'storefront' ); ?>
						<# } #>
						</a>
					<# } #>
				</div>
		...
		public function guided_tour_steps() {
			...
			$steps[] = array(
				'title'       => __( '欢迎来到Welcome to the Customizer', 'storefront' ),
				'message'     => sprintf( __( '在这里你可以控制整个商店的外观和感觉。Here you can control the overall look and feel of your store.%sTo get started, let\'s add your logo', 'storefront' ), PHP_EOL . PHP_EOL ),
