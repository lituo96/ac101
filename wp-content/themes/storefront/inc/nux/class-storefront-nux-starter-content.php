...
if ( ! class_exists( 'Storefront_NUX_Starter_Content' ) ) :
	...
	class Storefront_NUX_Starter_Content {
			...
			$starter_content = array(
				'posts' => array(
					'home' => array(
						...
						'post_content' => sprintf( esc_attr__( '这是你的主页，这是大多数访问者第一次参观你的商店时会看到的东西。%s您可以通过在仪表板中的“页面”菜单编辑“欢迎”页面来更改此文本。' ), PHP_EOL . PHP_EOL ),
					...
					'about' => array(
						...
						'post_content' => __( '你可能是一个想介绍你自己和你在这里工作的艺术家，或者也许你是一个有使命的企业来描述。', 'storefront' ),
					),
					'contact' => array(
						'post_type' => 'page',
						'post_title' => __( 'Contact', 'storefront' ),
						'post_content' => __( '这是一个带有一些基本联系信息的页面，如地址和电话号码。您也可以尝试插件来添加联系人表单。', 'storefront' ),
					),
					'blog'
				),
				'attachments' => array(
		...
		private function _starter_content_products() {
			$accessories_name        = esc_attr__( '配件Accessories', 'storefront' );
			$accessories_description = esc_attr__( '短范畴描述A short category description', 'storefront' );

			$hoodies_name            = esc_attr__( '帽衫Hoodies', 'storefront' );
			$hoodies_description     = esc_attr__( '短范畴描述A short category description', 'storefront' );

			$tshirts_name            = esc_attr__( 'T恤衫Tshirts', 'storefront' );
			$tshirts_description     = esc_attr__( '短范畴描述A short category description', 'storefront' );
